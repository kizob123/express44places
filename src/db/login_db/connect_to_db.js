...
module.exports ={
    login: function(email, password,res){
        ...
        conn.query(check_if_account_present,[email], function (err, result) {
            ...
            console.log(result);
            if (!result.length) {
                console.log(`There is no account tied to '${email}'`);
                res.json({
                    'error': `There is no account tied to '${email}'`
                })
            } else {
                ...
                conn.query(check_if_pass_correct,[email,password], function (err2, result2) {
                    ...
                    if (!result2.length) {
                        ...
                        let login = 'select fname, lname, email, password, business,phonenumber, routes from persons where email = ?'
    ...
    register:function(fname,lname, location, email,password,business,phone, routes,res){
